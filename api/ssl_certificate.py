...
class SSL_Certificate:
    ...
    async def _extract_certificate_details(self, cert: x509.Certificate) -> Dict:
        ...
        # Compute SHA1 Fingerprint - using serialization.Encoding instead of x509.Encoding
        try:
            cert_bin = cert.public_bytes(encoding=serialization.Encoding.DER)
            sha1_fingerprint = hashlib.sha1(cert_bin).hexdigest().upper()
            cert_details["Fingerprint"] = sha1_fingerprint
        except Exception as e:
            # Fallback: If public_bytes fails, try to get from the already parsed cert
            print(f"⚠️ Warning: Could not compute fingerprint: {e}")
            cert_details["Fingerprint"] = "N/A"
    ...
    async def __ssl_score(self, subject: str, issuer: str, expire: datetime, 
        ...
        if days_until_expiry is not None and 0 < days_until_expiry < 30:
            issues.append(Issue_Config.ISSUE_SSL_DAY_UNTIL_EXPIRES.format(days_until_expiry))
            suggestions.append(Issue_Config.SUGGESTION_SSL_DAY_UNTIL_EXPIRES)
        ...
        if domain_in_san:
            ...
            issues.append(Issue_Config.ISSUE_SSL_DOMAIN_NOT_FOUND.format(self.domain))
            suggestions.append(Issue_Config.SUGGESTION_SSL_DOMAIN_NOT_FOUND)
